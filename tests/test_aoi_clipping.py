...
def make_mock_connection(geometry_type="Polygon", boundary_coords=None):
    ...
    geometry = {"type": geometry_type, "coordinates": boundary_coords}
...
def test_clip_returns_tuple():
    ...
    result = clip_to_council_boundary(
...
def test_clip_invalid_gss_code_raises_value_error():
    ...
    with pytest.raises(ValueError):
        clip_to_council_boundary(
            band_array, scl_array, tile_metadata, "INVALID", mock_conn
        )
...
def test_clip_null_boundary_raises_value_error():
    ...
    with pytest.raises(ValueError):
        clip_to_council_boundary(
            band_array, scl_array, tile_metadata, "E06000021", mock_conn
        )
...
def test_clip_unsupported_geometry_raises_value_error():
    ...
    mock_cursor.fetchone.return_value = [
        json.dumps({"type": "Point", "coordinates": [500000, 5880000]})
    ]
...
def test_clip_handles_multipolygon():
    ...
    conn = make_mock_connection(
        geometry_type="MultiPolygon", boundary_coords=multipolygon_coords
    )
...
def test_clip_inside_unchanged_outside_zeroed():
    ...
    corner_boundary = [[[-100, 135], [75, 135], [75, 210], [-100, 210], [-100, 135]]]
