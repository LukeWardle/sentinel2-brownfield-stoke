...
def test_false_map_creation_with_mask(tmp_path):
    ...
    false_map_creation(
        rgb_array, str(tmp_path), mask=mask, original_shape=original_shape
    )
    png_files = list(tmp_path.glob("false_colour_map_*.png"))
    assert len(png_files) == 1
...
def test_report_creation_creates_pdf_file(tmp_path):
    ...
    report_creation(
        2,
        make_sorted_eigenvalues(),
        str(tmp_path),
        "E06000021",
        "2026-05-25",
        make_candidate_sites(),
...
def test_report_creation_large_change_detection(tmp_path):
    ...
    change_detection = {
        "added": [
            {"site_reference": f"NEW{i:03d}", "name_address": f"New Site {i}"}
            for i in range(20)
        ],
        "removed": [
            {"site_reference": f"OLD{i:03d}", "name_address": f"Old Site {i}"}
            for i in range(15)
        ],
    }
    report_creation(
