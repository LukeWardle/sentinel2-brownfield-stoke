...
def mock_data():
    ...
    return {
        ...
        "clipped_bands": np.random.randint(
            0, 10000, (height, width, 10), dtype=np.uint16
        ),
...
def get_patches(mock_data, safe_path="/tmp/test.SAFE"):
    ...
    return {
        ...
        "src.main.get_tile_metadata": MagicMock(
            return_value=mock_data["tile_metadata"]
        ),
        "src.main.clip_to_council_boundary": MagicMock(
            return_value=(mock_data["clipped_bands"], mock_data["clipped_scl"])
        ),
        "src.main.mask_nodata": MagicMock(
            return_value=(
                mock_data["masked_array"],
                mock_data["mask"],
                mock_data["original_shape"],
            )
        ),
        "src.main.validate_bands": MagicMock(return_value=None),
        "src.main.validate_quality": MagicMock(return_value=None),
        "src.main.normalise_band_array": MagicMock(
            return_value=mock_data["normalised"]
        ),
        ...
        "src.main.spectral_decomposition": MagicMock(
            return_value=(mock_data["eigenvalues"], mock_data["eigenvectors"])
        ),
        "src.main.sort_variance": MagicMock(
            return_value=(mock_data["eigenvalues"], mock_data["eigenvectors"])
        ),
        "src.main.cumulative_variance_for_k": MagicMock(return_value=3),
        "src.main.project": MagicMock(return_value=mock_data["X_reduced"]),
        "src.main.group_pixels_for_candidate_sites": MagicMock(
            return_value=mock_data["candidate_groups"]
        ),
        "src.main.calculate_site_properties": MagicMock(
            return_value=mock_data["site_properties"]
        ),
        ...
        "src.main.detect_register_changes": MagicMock(
            return_value=mock_data["change_detection"]
        ),
...
def test_run_pipeline_raises_for_invalid_gss_code(tmp_path, mock_data):
    ...
    with pytest.raises(ValueError):
        run_with_mocks(
            ...
            mock_data,
            overrides={
                "src.main.validate_council_boundary_gss": MagicMock(
                    side_effect=ValueError("Invalid GSS code")
                )
            },
        )
...
def test_run_pipeline_raises_for_no_products_found(tmp_path, mock_data):
    ...
    with pytest.raises(ValueError):
        run_with_mocks(
            ...
            mock_data,
            overrides={
                "src.main.search_products": MagicMock(
                    side_effect=ValueError("No products found")
                )
            },
        )
...
def test_run_pipeline_raises_for_failed_download(tmp_path, mock_data):
    ...
    with pytest.raises(ValueError):
        run_with_mocks(
            ...
            mock_data,
            overrides={
                "src.main.download_safe": MagicMock(
...
def test_run_pipeline_raises_for_band_validation_failure(tmp_path, mock_data):
    ...
    with pytest.raises(ValueError):
        run_with_mocks(
            ...
            mock_data,
            overrides={
                "src.main.validate_bands": MagicMock(
                    side_effect=ValueError("Band validation failed")
                )
            },
        )
...
def test_run_pipeline_raises_for_quality_validation_failure(tmp_path, mock_data):
    ...
    with pytest.raises(ValueError):
        run_with_mocks(
            ...
            mock_data,
            overrides={
                "src.main.validate_quality": MagicMock(
                    side_effect=ValueError("Cloud cover too high")
                )
            },
        )
...
def test_run_pipeline_stores_failure_status_on_error(tmp_path, mock_data):
    ...
    with pytest.raises(ValueError):
        run_with_mocks(
            ...
            overrides={
                "src.main.search_products": MagicMock(
                    side_effect=ValueError("No products")
                ),
...
def test_run_pipeline_calls_register_matching(tmp_path, mock_data):
    ...
    mocks = run_with_mocks("E06000021", "2026-05-25", str(tmp_path), mock_data)
    assert mocks["src.main.match_candidate_to_register"].call_count == len(
        mock_data["site_properties"]
    )
...
def test_run_pipeline_stores_success_status_on_completion(tmp_path, mock_data):
    ...
    run_with_mocks(
        ...
        str(tmp_path),
        mock_data,
        overrides={"src.main.store_pipeline_metadata": mock_store},
    )
    mock_store.assert_called_once()
