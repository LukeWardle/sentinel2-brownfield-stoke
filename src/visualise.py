...
def report_creation(
    ...
    if (
        not isinstance(change_detection, dict)
        or "added" not in change_detection
        or "removed" not in change_detection
    ):
        raise ValueError(
            "change_detection must be a dict with 'added' and 'removed' keys"
        )
    ...
    if unmatched:
        story.append(
            Paragraph("Potential Unregistered Brownfield Sites", section_style)
        )
        story.append(
            Paragraph(
        ...
        unmatched_table = Table(
            unmatched_data, colWidths=[2 * cm, 4 * cm, 3 * cm, 3.5 * cm, 3.5 * cm]
        )
        unmatched_table.setStyle(
            TableStyle(
                [
    ...
    if removed:
        story.append(Spacer(1, 0.2 * cm))
        story.append(
            Paragraph("Sites Likely Developed (removed from register):", body_style)
        )
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 1),
                        (-1, -1),
                        [colors.HexColor("#f2fdf5"), colors.white],
                    ),
                    ("GRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#cccccc")),
                    ("PADDING", (0, 0), (-1, -1), 5),
                ]
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 1),
                        (-1, -1),
                        [colors.HexColor("#f2f8fd"), colors.white],
                    ),
                    ("GRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#cccccc")),
                    ("PADDING", (0, 0), (-1, -1), 5),
                ]
    ...
    story.append(
        Paragraph(
            "Sentinel-2 L2A imagery via Copernicus Data Space "
...
def create_interactive_map(
    candidate_sites: list, output_dir: str, gss_code: str
) -> None:
    """
    ...
    <div style="position: fixed; bottom: 30px; left: 30px; z-index: 1000;
        ...
        <b>Brownfield Candidate Sites</b><br>
