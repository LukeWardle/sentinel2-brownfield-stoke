...
**[DECISION] min_pixels=5 is PROVISIONAL pending calibration against post-FND-1 true counts**
FND-1 stopped counting dilation-border pixels toward group membership. The previous min_pixels=10 was calibrated implicitly against dilation-INFLATED counts and is therefore void — it no longer refers to the quantity it was tuned on. Dropping to min_pixels=5 (0.2 ha at 20 m, the module default) recovers the site count from 25 to 90 on the May 2026 Stoke image (78 after exclusion, 18 register matches). This value is chosen as a defensible physical minimum parcel size and was fixed BEFORE observing recall, but it is not a derived calibration: the true-count distribution is steep at this point (10->5 nearly quadruples the count), so the exact threshold is load-bearing and has not yet been observed against the distribution. min_pixels=5 ships as a provisional value; principled calibration against the true-count distribution — cross-referenced with register recall and the P1-1 labelling pilot — is tracked as a separate ticket. The earlier headline figure of 218 candidate sites and 39 register matches was measured under dilation-inflated counts with no exclusion filter and is not comparable to post-FND-1 numbers.
